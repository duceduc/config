...
class AmazonPriceTrackerConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_import_wishlist(
        ...
        if user_input is not None:
            ...
            if not (wishlist_match := _WISHLIST_RE.search(url)):
...
class AmazonPriceTrackerOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_init(
        ...
        if user_input is not None:
            name = user_input["name"].strip()
            # Both keys are always written, including as None: falling back to
            # entry.data on a missing key would resurrect the threshold the user
            # has just cleared.
            return self.async_create_entry(
                title=name,
                data={
